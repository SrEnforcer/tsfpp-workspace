/**
 * Examples for §7 — Naming (Rules 7.1–7.7)
 * See ../CODING_STANDARD.md §7
 */
...
// ─── Rule 7.7 — Effectful wrappers use `with` prefix ─────────────────────────
//
// SHOULD: Decorator/middleware-style wrappers should start with `with`.

type Handler<A, E> = () => Promise<ParseResult2<A, E>>

const withLogging = <A, E>(label: string, handler: Handler<A, E>): Handler<A, E> =>
  async () => {
    console.log(`[${label}] start`)
    const result = await handler()
    console.log(`[${label}] done`, result.ok)
    return result
  }

const withRetry = <A, E>(attempts: number, handler: Handler<A, E>): Handler<A, E> =>
  async () => {
    const run = async (remaining: number): Promise<ParseResult2<A, E>> => {
      const result = await handler()
...
/* BAD: wrapper intent hidden behind generic names.
const addLogging = (...) => ...
const decorate = (...) => ...
*/
