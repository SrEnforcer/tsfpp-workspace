/**
 * Examples for §2 — Immutability (Rules 2.1–2.5)
 * See ../CODING_STANDARD.md §2 and ../rationale/02-immutability.md
 */
...
// ─── Rule 2.5 — Use `as const` for literal narrowing/config tables ───────────
//
// MUST: Use `as const` to preserve literal precision and readonly values.
...
/* BAD: without `as const`, literals widen and lose exhaustiveness leverage.
const httpMethods = ['GET', 'POST', 'PUT', 'DELETE'] // string[]
const retryPolicy = { retries: 3, mode: 'exponential' } // retries: number, mode: string
*/

// ─── Exports ─────────────────────────────────────────────────────────────────
export type { AstAttr, AstNode, TaggedItem, Config, Status, HttpMethod }
export {
  countChildren, findByKind, withNewItem, withoutId, sortedByTag, updatedTag, withPort,
  getLabel, items, numbers, total, doubled,
  httpMethods, retryPolicy, isWriteMethod,
}
