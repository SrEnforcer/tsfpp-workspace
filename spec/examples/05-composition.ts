 ...
 * See ../CODING_STANDARD.md §5 and ../rationale/04-control-flow.md
 */
...
//
// In production code, import from @tsfpp/prelude (and Remeda where needed).
...
// Real usage: import { pipe } from '@tsfpp/prelude'.
...
// GOOD: Prelude/Remeda-style pipe (import from the project prelude in real code)
// const result = R.pipe(
//   R.filter(isActive),
...
import { flow, prop, split, head } from '@tsfpp/prelude'
const getActiveNames = R.pipe(
  R.filter(prop('active')),
  R.map(prop('name')),
  R.map(flow(split(' '), head))   // what type does this operate on now?
)
*/
